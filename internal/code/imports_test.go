...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/bewolv/gqlgen/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/bewolv/gqlgen/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/bewolv/gqlgen/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/bewolv/gqlgen/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
...
func TestNameForPackage(t *testing.T) {
	assert.Equal(t, "api", NameForPackage("github.com/bewolv/gqlgen/api"))

	// does not contain go code, should still give a valid name
	assert.Equal(t, "docs", NameForPackage("github.com/bewolv/gqlgen/docs"))
